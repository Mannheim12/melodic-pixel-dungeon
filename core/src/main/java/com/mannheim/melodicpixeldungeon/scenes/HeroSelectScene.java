...
import com.mannheim.melodicpixeldungeon.actors.hero.HeroClass;
...
import com.mannheim.melodicpixeldungeon.ui.ExitButton;
import com.mannheim.melodicpixeldungeon.ui.IconButton;
...
import com.mannheim.melodicpixeldungeon.windows.WndTextInput;
import com.mannheim.melodicpixeldungeon.utils.DungeonSeed;
import com.mannheim.melodicpixeldungeon.windows.WndChallenges;
import com.mannheim.melodicpixeldungeon.windows.WndHeroInfo;
...
public class HeroSelectScene extends PixelScene {
	...
	public void create() {
		...
		infoButton = new IconButton(Icons.get(Icons.INFO)){
			...
			protected void onClick() {
				...
				MelodicPixelDungeon.scene().addToFront(w);
			}
	...
	protected void onBackPressed() {
		if (btnExit.active){
			MelodicPixelDungeon.switchScene(TitleScene.class);
		} else {
			super.onBackPressed();
		}
	}
	...
	private class HeroBtn extends StyledButton {
		...
		protected void onClick() {
			...
			if( !cl.isUnlocked() ){
				MelodicPixelDungeon.scene().addToFront( new WndMessage(cl.unlockMsg()));
			...
			if (DeviceCompat.isDebug() || Badges.isUnlocked(Badges.Badge.VICTORY)){
				StyledButton seedButton = new StyledButton(Chrome.Type.BLANK, Messages.get(HeroSelectScene.class, "custom_seed"), 6){
					...
					protected void onClick() {
						...
						MelodicPixelDungeon.scene().addToFront(new WndTextInput(Messages.get(HeroSelectScene.class, "custom_seed_title"),
								...
								if (positive && seed != -1){
									...
									for (GamesInProgress.Info info : GamesInProgress.checkAll()){
										if (info.customSeed.isEmpty() && info.seed == seed){
											SPDSettings.customSeed("");
											icon.resetColor();
											MelodicPixelDungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "custom_seed_duplicate")));
											return;
										}
									}
				...
				StyledButton dailyButton = new StyledButton(Chrome.Type.BLANK, Messages.get(HeroSelectScene.class, "daily"), 6){
					...
					protected void onClick() {
						...
						if (diff > 24*HOUR){
							MelodicPixelDungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "daily_unavailable_long", (diff / DAY)+1)));
							return;
						}

						for (GamesInProgress.Info game : GamesInProgress.checkAll()){
							if (game.daily){
								MelodicPixelDungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "daily_existing")));
								return;
							}
						}
						...
						MelodicPixelDungeon.scene().addToFront(new WndOptions(
								icon,
				...
				StyledButton challengeButton = new StyledButton(Chrome.Type.BLANK, Messages.get(WndChallenges.class, "title"), 6){
					@Override
					protected void onClick() {
						MelodicPixelDungeon.scene().addToFront(new WndChallenges(SPDSettings.challenges(), true) {
