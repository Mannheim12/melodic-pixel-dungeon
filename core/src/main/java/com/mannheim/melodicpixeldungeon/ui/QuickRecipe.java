...
package com.mannheim.melodicpixeldungeon.ui;

import com.mannheim.melodicpixeldungeon.Dungeon;
import com.mannheim.melodicpixeldungeon.MelodicPixelDungeon;
import com.mannheim.melodicpixeldungeon.items.ArcaneResin;
import com.mannheim.melodicpixeldungeon.items.Generator;
import com.mannheim.melodicpixeldungeon.items.Item;
import com.mannheim.melodicpixeldungeon.items.LiquidMetal;
import com.mannheim.melodicpixeldungeon.items.Recipe;
import com.mannheim.melodicpixeldungeon.items.bombs.Bomb;
import com.mannheim.melodicpixeldungeon.items.food.Blandfruit;
import com.mannheim.melodicpixeldungeon.items.food.Food;
import com.mannheim.melodicpixeldungeon.items.food.MeatPie;
import com.mannheim.melodicpixeldungeon.items.food.MysteryMeat;
import com.mannheim.melodicpixeldungeon.items.food.Pasty;
import com.mannheim.melodicpixeldungeon.items.food.StewedMeat;
import com.mannheim.melodicpixeldungeon.items.potions.AlchemicalCatalyst;
import com.mannheim.melodicpixeldungeon.items.potions.Potion;
import com.mannheim.melodicpixeldungeon.items.potions.brews.BlizzardBrew;
import com.mannheim.melodicpixeldungeon.items.potions.brews.CausticBrew;
import com.mannheim.melodicpixeldungeon.items.potions.brews.InfernalBrew;
import com.mannheim.melodicpixeldungeon.items.potions.brews.ShockingBrew;
import com.mannheim.melodicpixeldungeon.items.potions.elixirs.ElixirOfAquaticRejuvenation;
import com.mannheim.melodicpixeldungeon.items.potions.elixirs.ElixirOfArcaneArmor;
import com.mannheim.melodicpixeldungeon.items.potions.elixirs.ElixirOfDragonsBlood;
import com.mannheim.melodicpixeldungeon.items.potions.elixirs.ElixirOfHoneyedHealing;
import com.mannheim.melodicpixeldungeon.items.potions.elixirs.ElixirOfIcyTouch;
import com.mannheim.melodicpixeldungeon.items.potions.elixirs.ElixirOfMight;
import com.mannheim.melodicpixeldungeon.items.potions.elixirs.ElixirOfToxicEssence;
...
import com.mannheim.melodicpixeldungeon.items.spells.BeaconOfReturning;
import com.mannheim.melodicpixeldungeon.items.spells.CurseInfusion;
import com.mannheim.melodicpixeldungeon.items.spells.FeatherFall;
import com.mannheim.melodicpixeldungeon.items.spells.MagicalInfusion;
import com.mannheim.melodicpixeldungeon.items.spells.PhaseShift;
import com.mannheim.melodicpixeldungeon.items.spells.ReclaimTrap;
import com.mannheim.melodicpixeldungeon.items.spells.Recycle;
import com.mannheim.melodicpixeldungeon.items.spells.SummonElemental;
import com.mannheim.melodicpixeldungeon.items.spells.TelekineticGrab;
import com.mannheim.melodicpixeldungeon.items.spells.WildEnergy;
...
public class QuickRecipe extends Component {
	...
	public QuickRecipe(Recipe r, ArrayList<Item> inputs, final Item output) {
		...
		for (final Item in : inputs) {
			...
			curr = new ItemSlot(in) {
				...
				protected void onClick() {
					MelodicPixelDungeon.scene().addToFront(new WndInfoItem(in));
				}
			};
		...
		if (hasInputs) {
			...
			if (!(MelodicPixelDungeon.scene() instanceof AlchemyScene)) {
				arrow.enable(false);
			}
		} else {
		...
		this.output = new ItemSlot(output){
			@Override
			protected void onClick() {
				MelodicPixelDungeon.scene().addToFront(new WndInfoItem(output));
			}
		};
		if (!hasInputs){
	...
	public class arrow extends IconButton {
		...
		protected void onClick() {
			...
			((AlchemyScene) MelodicPixelDungeon.scene()).populate(ingredients, Dungeon.hero.belongings);
		}
