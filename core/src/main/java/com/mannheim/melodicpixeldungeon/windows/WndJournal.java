...
import com.mannheim.melodicpixeldungeon.ui.QuickRecipe;
...
import com.mannheim.melodicpixeldungeon.ui.ScrollingListPane;
		...
		private void updateList(){
			...
			for (Document doc : Document.values()){
				...
				ScrollingListPane.ListItem item = new ScrollingListPane.ListItem(
					...
					public boolean onClick(float x, float y) {
						if (inside( x, y ) && found) {
							MelodicPixelDungeon.scene().addToFront( new WndDocument( doc ));
							return true;
						} else {
							return false;
						}
